...
const modelReport = connection.model('reported', {
...
module.exports = {
    ...
    onInsert(value) {
        return new Promise((resolve, reject) => {
            ...
            const newManage = new modelReport({
    ...
    findOne(value) {
        return new Promise((resolve, reject) => {
            modelReport.find({ _id: value }, { _id: 0 }).then(result => {
                resolve(result[0])
            }).catch(err => reject(err))
        })
    },
    ...
    findByName(value) {
        return new Promise((resolve, reject) => {
            ...
            modelReport.findOne({ name: value.name }).then(result => {
    ...
    findAll(value) {
        return new Promise((resolve, reject) => {
            modelReport.find().sort({ "_id": 1 }).then(result => {
                resolve(result)
            }).catch(err => reject(err))
        })
    },
    ...
    onUpdateData(value) {
        ...
        return new Promise((resolve, reject) => {
            var conditions = { _id: value._id }
                ...
                options = { multi: true };
            modelReport.updateOne(conditions, update, options).then(result => {
                resolve(result)
            }).catch(err => reject(err))
        })
    },
